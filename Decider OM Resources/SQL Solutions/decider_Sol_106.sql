-- CREATE DATABASE FOR /home/centos/orm_synthesizer/models/decider/ImplSolution/decider_Sol_106.sql
...
`department` int,
`participantID` int NOT NULL,
KEY `FK_Developer_participantID_idx` (`participantID`),
PRIMARY KEY (`participantID`)
);
...
`period` int,
`participantID` int NOT NULL,
KEY `FK_Viewer_participantID_idx` (`participantID`),
PRIMARY KEY (`participantID`)
);
...
ALTER TABLE `Developer`
  ADD CONSTRAINT `FK_Developer_participantID` FOREIGN KEY (`participantID`) REFERENCES `Participant` (`participantID`) ON DELETE CASCADE ON UPDATE CASCADE;

ALTER TABLE `Viewer`
  ADD CONSTRAINT `FK_Viewer_participantID` FOREIGN KEY (`participantID`) REFERENCES `Participant` (`participantID`) ON DELETE CASCADE ON UPDATE CASCADE;
