...
import os
i = 0
for root, dirs, files in os.walk('/Users/rashedhasan/Desktop/UNL/Research/Object relational mapping/Step 5 - Abstraction/OM-Solution_Mapping/OM-ML_Research/Store Management/Schema_Process_NP'):
    for file in files:
        #each file
        ...
        if extension == '.sql':
            ...
            #   -------------------------------------------------------------------------------
             ...
             # ------------ ----------------- ------------- -  - --- ----
                ...
                #
                # if f'CREATE TABLE `{assoc11}` (' in mylist[j]:
                #     mylist[j] = (f"CREATE TABLE `{assoc11_0}` (")
                #     a422= mylist[j]
                #     file_object.write(a422)
                #     file_object.write("\n")

                #
                # if f'KEY `FK_{assoc11}_{c3_at1}_idx` (`{c3_at1}`),' in mylist[j]:
                #     mylist[j] = (f"KEY `FK_{assoc11_0}_{c3_at1_0}_idx` (`{c3_at1_0}`),")
                #     a423= mylist[j]
                #     file_object.write(a423)
                #     file_object.write("\n")

                #
                # if f'KEY `FK_{assoc11}_{c1_at1}_idx` (`{c1_at1}`),' in mylist[j]:
                #     mylist[j] = (f"KEY `FK_{assoc11_0}_{c1_at1_0}_idx` (`{c1_at1_0}`),")
                #     a424= mylist[j]
                ...
                #
                # if f'ALTER TABLE `{assoc11}`' in mylist[j]:
                #     mylist[j] = (f"ALTER TABLE `{assoc11_0}`")
                #     a425= mylist[j]
                #     file_object.write(a425)
                #     file_object.write("\n")


                # if f'ADD CONSTRAINT `FK_{assoc11}_{c3_at1}` FOREIGN KEY (`{c3_at1}`) REFERENCES `{class3_name}` (`{c3_at1}`) ON DELETE CASCADE ON UPDATE CASCADE,' in mylist[j]:
                ...
                #
                #
                # if f'ADD CONSTRAINT `FK_{assoc11}_{c1_at1}` FOREIGN KEY (`{c1_at1}`) REFERENCES `{class1_name}` (`{c1_at1}`) ON DELETE CASCADE ON UPDATE CASCADE;' in mylist[j]:
                #     mylist[j] = (f"ADD CONSTRAINT `FK_{assoc11_0}_{c1_at1_0}` FOREIGN KEY (`{c1_at1_0}`) REFERENCES `{class1_name_0}` (`{c1_at1_0}`) ON DELETE CASCADE ON UPDATE CASCADE;")
                #     a427= mylist[j]
