-- CREATE DATABASE FOR /home/centos/orm_synthesizer/models/Canteen/ImplSolution/Canteen_Sol_3059.sql

USE Canteen_Sol_3059;
--
...
`UserId` int NOT NULL, 
KEY `FK_Salesman_UserId_idx` (`UserId`),
PRIMARY KEY (`UserId`)
);
...
--
-- Table structure for table FoodLibraryDbAssociation
--

CREATE TABLE `FoodLibraryDbAssociation` (
`ItemId` int NOT NULL, 
`PaymentId` int NOT NULL, 
KEY `FK_FoodLibraryDbAssociation_ItemId_idx` (`ItemId`),
KEY `FK_FoodLibraryDbAssociation_PaymentId_idx` (`PaymentId`),
PRIMARY KEY (`ItemId`,`PaymentId`)
);
...
`ProductId` int NOT NULL, 
KEY `FK_Product_CanteenId_idx` (`CanteenId`),
...
ALTER TABLE `Salesman`
  ADD CONSTRAINT `FK_Salesman_UserId` FOREIGN KEY (`UserId`) REFERENCES `User` (`UserId`) ON DELETE CASCADE ON UPDATE CASCADE;
...
ALTER TABLE `FoodLibraryDbAssociation`
  ADD CONSTRAINT `FK_FoodLibraryDbAssociation_ItemId` FOREIGN KEY (`ItemId`) REFERENCES `Food` (`ItemId`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `FK_FoodLibraryDbAssociation_PaymentId` FOREIGN KEY (`PaymentId`) REFERENCES `Payment` (`PaymentId`) ON DELETE CASCADE ON UPDATE CASCADE;

ALTER TABLE `Product`
  ADD CONSTRAINT `FK_Product_CanteenId` FOREIGN KEY (`CanteenId`) REFERENCES `Canteen` (`CanteenId`) ON DELETE CASCADE ON UPDATE CASCADE;
